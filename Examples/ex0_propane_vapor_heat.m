...
% Examples of NFP use with propane
...
close all
 ...
 NFP('C3H8','Tsat_p',10) % K
 
 % saturation pressure at 200 K
 NFP('C3H8','Psat_T',200) % bar
 
 % saturation pressure at 300 K
 NFP('C3H8','Psat_T',300) % bar

 % latent l->v heat a 20 bar
 NFP('C3H8','hv_p',20)-NFP('C3H8','hl_p',20) % kJ/kg
 
 % entropy of saturated liquid and vapour at 12 bar
 NFP('C3H8','sl_p',12) % kJ/kgK
 NFP('C3H8','sv_p',12) % kJ/kgK
 
 % latent vaporisation heat vs. sat pressure
 pv=linspace(1,NFP('C3H8','pcrit'),40); % 40 values 
 for i=1:length(pv)
     dhv(i)= NFP('C3H8','hv_p',pv(i))-NFP('C3H8','hl_p',pv(i));
 end
...
NFP('C3H8','h_pt',5,400) % Enthalpy kJ/kgK
NFP('C3H8','s_pt',5,400) % Entropy kJ/kgK
NFP('C3H8','u_pt',5,400) % Internal energy kJ/kg
NFP('C3H8','v_pt',5,400) % Volume m^3/kg
